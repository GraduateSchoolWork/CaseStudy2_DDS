...
```{r}
attrition.df$JobLevel <- gsub(1, "1", attrition.df$JobLevel)
attrition.df$JobLevel <- gsub(2, "2", attrition.df$JobLevel)
attrition.df$JobLevel <- gsub(3, "3", attrition.df$JobLevel)
attrition.df$JobLevel <- gsub(4, "4", attrition.df$JobLevel)
attrition.df$JobLevel <- gsub(5, "5", attrition.df$JobLevel)
attrition.df$JobLevel <- as.character(attrition.df$JobLevel)

# not as good
Model1_fit = lm(MonthlyIncome~JobLevel, data = attrition.df)
summary(Model1_fit)
preds <- predict(Model1_fit)
hist(Model1_fit$residuals, col = "blue", main = "Histogram of Residuals")
plot(Model1_fit$fitted.values,Model1_fit$residuals, main = "Plot of Residuals v. Fitted Values")
attrition.df %>% ggplot(aes(x = JobLevel, y = MonthlyIncome)) + geom_point() + geom_line(data = attrition.df, aes(x = JobLevel, y = preds, col = "red"))

test1 <- predict(Model1_fit, noSalary, interval = "confidence") 
test1 <- as.data.frame(test1)
noSalary$MonthlyIncome <- test1[,1]
...
attriton.df3 %>% ggplot(aes(x = JobLevel, y = MonthlyIncome)) + geom_point() + geom_line(data = attriton.df3, aes(x = JobLevel, y = preds2, col = "red"))
...
noSalary %>% ggplot(aes(x = JobLevel, y = Salary2)) + geom_point() + geom_line(data = noSalary, aes(x = JobLevel, y = Salary2 , col = "red"))
...
attriton.df4 %>% ggplot(aes(x = JobLevel, y = MonthlyIncome)) + geom_point() + geom_line(data = attriton.df4, aes(x = JobLevel, y = preds3, col = "red"))
...
```{r}
ggplot(attrition.df, aes(x=PerformanceRating, y=MonthlyIncome)) +
  geom_point() +
  coord_flip()
```